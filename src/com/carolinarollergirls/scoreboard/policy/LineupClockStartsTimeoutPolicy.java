...
public class LineupClockStartsTimeoutPolicy extends AbstractClockTimeChangePolicy
{
  ...
  public static final String DESCRIPTION = "This starts a Timeout based on the Lineup clock.  When the Lineup clock reaches or exceeds the trigger value (by default 30 seconds), the Lineup clock is stopped and reset (to avoid possible accidental triggers of this policy) and a Timeout is started.  If the period rollback value is not zero, the Period clock time will be rolled back by that value.  The period rollback time is intended to be used as a 'buffer' so the jam can start a couple seconds after the 30-second lineup to allow for slight differences between the scoreboard operator's lineup clock time and the NSO scorekeeper's lineup clock time, but for lineups that go 'too far' past the 30-second time a timeout will be called and the period clock 'rolled back' to exactly the time it was at when the lineup clock hit 30 seconds.  So for example, if the trigger time was 35 seconds, the rollback value should be 5 seconds.  By default, this policy is disabled.  Also note that if the Lineup clock's minimum time is greater than or equal to the trigger time (if the Lineup clock is counting up; if counting down, the maximum time is less than or equal to the trigger time), this policy will not operate, to avoid an infinite loop.";
}
