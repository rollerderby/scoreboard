...
public class ClientsImplTests {
    ...
    protected void fuzzSet(String path) {
        List<JSONSet> sets = new ArrayList<JSONSet>(Arrays.asList(
                    new JSONSet(path, null, null),
                    new JSONSet(path, "", null),
                    new JSONSet(path, "abc", null),
                    new JSONSet(path, "20", null),
                    // WS can use these flags.
                    new JSONSet(path, "answer", Flag.CHANGE),
                    new JSONSet(path, "42", Flag.CHANGE),
                    new JSONSet(path, null, Flag.RESET)
                ));
        for (JSONSet s : sets) {
            try {
                ScoreBoardJSONSetter.set(sb, Collections.singletonList(s));
            } catch (Exception e) {
    ...
    public void testWritesPrevented() {
        ...
        // This should work.
        ScoreBoardJSONSetter.set(sb, Collections.singletonList(
                                     new JSONSet(dId+".Comment", "new comment", null)));
