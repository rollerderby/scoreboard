...
Timeline.GregorianEtherPainter.prototype.initialize = function(band, timeline) {
    ...
    if (band._bgColor)
        this._backgroundLayer.style.background = band._bgColor;
...
/*==================================================
 *  Marked Ether Painter
 *==================================================
 */
 
Timeline.MarkedEtherPainter = function(params) {
    this._params = params || {};
    this._theme = this._params.theme || Timeline.getDefaultTheme();
    this._labels = this._params.labels || new Array();
};

Timeline.MarkedEtherPainter.prototype.initialize = function(band, timeline) {
    this._band = band;
    this._timeline = timeline;
...
Timeline.MarkedEtherPainter.prototype.setHighlight = function(startDate, endDate) {
    this._highlight.position(startDate, endDate);
};

Timeline.MarkedEtherPainter.prototype.paint = function() {
    if (this._markerLayer) {
        this._band.removeLayerDiv(this._markerLayer);
    }
    this._markerLayer = this._band.createLayerDiv(100);
    ...
    for (var l in this._labels) {
        if (this._labels[l].date.getTime() < this._band.getMinDate().getTime() || this._labels[l].date.getTime() > this._band.getMaxDate().getTime())
            continue;
        var mark = this._intervalMarkerLayout.createIntervalMarker(
            this._labels[l].date, labeller, SimileAjax.DateTime.YEAR, this._markerLayer, this._lineLayer);
        mark.innerHTML = this._labels[l].label;
    }
    this._markerLayer.style.display = "block";
    this._lineLayer.style.display = "block";
};

Timeline.MarkedEtherPainter.prototype.softPaint = function() {
};

Timeline.MarkedEtherPainter.prototype.zoom = function(netIntervalChange) { };
