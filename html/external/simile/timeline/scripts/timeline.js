...
Timeline.createBandInfo = function(params) {
    ...
    var ether = params.ether || new Timeline.LinearEther({ 
    ...
    var etherPainter = params.etherPainter || new Timeline.GregorianEtherPainter({
...
Timeline._Impl.prototype._distributeWidths = function() {
    ...
    for (var i = 0; i < this._bands.length; i++) {
        ...
        if (typeof widthString == 'string') {
          ...
          if (x > 0) {
              var percent = Number(widthString.substr(0, x));
...
Timeline._Impl.prototype.shiftOK = function(index, shift) {
    ...
    for (var i = 0; i < this._bands.length && !ok; i++) {
       var band = this._bands[i];
       if (going_back) {
           ok = (this.timeline_start
                ? (i == index ? band.getMinVisibleDateAfterDelta(shift) : band.getMinVisibleDate()) >= this.timeline_start
                : true);
       } else {
           ok = (this.timeline_stop
                ? (i == index ? band.getMaxVisibleDateAfterDelta(shift) : band.getMaxVisibleDate()) <= this.timeline_stop
                : true);
       }	
    }
