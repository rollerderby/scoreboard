...
function setupMainDiv(div) {
	...
	_crgUtils.bindAndRun($(window), "resize", function() {
		...
		console.log(aspect16x9);
		div.css(aspect16x9).css("fontSize", aspect16x9.height).css("top", "0px");
	});
}
...
$sb(function() {
	...
	$.each( [ "1", "2" ], function(i, t) {
		...
		team.$sb("Name")
			.$sbElement("#MainBar>div.Team"+t+">div.Name>span.Name>a.Name");
		team.$sb("AlternateName(overlay).Name")
			.$sbElement("#MainBar>div.Team"+t+">div.Name>span.Name>a.AlternateName");
		team.$sb("AlternateName(overlay_teamcolor).Name")
			.$sbElement("#MainBar>div.Team"+t+">div.Name>a.AlternateNameTeamColor");
		team.$sb("Score")
		...
		team.$sbBindAddRemoveEach("AlternateName", function(event, node) {
			...
			else if ($sb(node).$sbId == "overlay_teamcolor")
				$sb(node).$sb("Name").$sbBindAndRun("sbchange", function(event, val) {
					$("#MainBar>div.Team"+t+">div.Name")
						.toggleClass("AlternateNameTeamColor", ($.trim(val) != ""));
				});
		}, function(event, node) {
			if ($sb(node).$sbId == "overlay")
